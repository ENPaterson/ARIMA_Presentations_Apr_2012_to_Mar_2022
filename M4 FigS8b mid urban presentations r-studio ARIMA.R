...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=mid_urban_presentations_n))+
...
ARIMA %>% 
  ggplot(aes(x=month, y=mid_urban_presentations_n))+
  geom_line()+
  labs(
    y = "Presentations (n)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=mid_urban_presentations_n))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
Apr2012_Feb2022 <- ARIMA$mid_urban_presentations_n[1:95]      

PANDEMIC2020_2022 <- ARIMA$mid_urban_presentations_n[96:120]   
...
forecast_arima_Apr2012_Feb2022 <- forecast(fit_arima_Apr2012_Feb2022,
                                           h=25) # forecast 25 months (Mar 2020 to Mar 2022. Be aware, this is 2 years and one month. We are inlcuding the month of March 3 times, for 2020, 2021, an 2022, and every other month only twice)
autoplot(forecast_arima_Apr2012_Feb2022) # additional argument include = 12, would include only the last 12 months in precovid time frame
...
ggplot(data = forecast_arima_Apr2012_Feb2022_df,
  ...
  ylim(0, 850)
ggsave("M4_7b.pdf", width = 15, height = 15)
