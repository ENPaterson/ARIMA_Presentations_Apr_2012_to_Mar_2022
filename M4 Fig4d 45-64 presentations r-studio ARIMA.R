...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=presentations_n_4564))+
...
ARIMA %>% 
  ggplot(aes(x=month, y=presentations_n_4564))+
  geom_line()+
  labs(
    y = "Presentations (n)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=presentations_n_4564))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
Apr2012_Feb2022 <- ARIMA$presentations_n_4564[1:95]      

PANDEMIC2020_2022 <- ARIMA$presentations_n_4564[96:120]   
...
ggplot(data = forecast_arima_Apr2012_Feb2022_df,
  ...
  ylim(0, 750)
ggsave("M4_4d.pdf", width = 15, height = 15)
