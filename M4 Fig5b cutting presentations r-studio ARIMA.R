...
ARIMA  %>% 
  ...
  ggplot(aes(x=as.factor(year), y=cutting_presentations_n))+
...
ARIMA %>% 
  ggplot(aes(x=month, y=cutting_presentations_n))+
  geom_line()+
  labs(
    y = "Presentations (n)"
  )
...
ARIMA  %>% 
  ...
  ggplot(aes(x=c_month, y=cutting_presentations_n))+
  geom_line()+
  facet_wrap(~year)+
  labs(
    x = "Month",
...
Apr2012_Feb2022 <- ARIMA$cutting_presentations_n[1:95]      

PANDEMIC2020_2022 <- ARIMA$cutting_presentations_n[96:120]   
...
ggplot(data = forecast_arima_Apr2012_Feb2022_df,
  ...
  ylim(0, 750)
ggsave("M4_5b.pdf", width = 15, height = 15)
